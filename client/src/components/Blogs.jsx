           ...
           {currentBlogs.length > 0 ? (
            currentBlogs.map((blog) =>(
                <div key={blog._id} className='bg-gray-200 p-4 '>
                    <img src={`https://blog-app-1-4i8q.onrender.com${blog.image}`} className='w-full h-32 object-cover'></img>
                 <h3>{blog.title}</h3>
                 <div
  className="text-gray-700 text-sm"
  dangerouslySetInnerHTML={{
    __html: DOMPurify.sanitize(blog.content.substring(0, 50) + "..."),
  }}
/>
       ...
       <div className='w-full md:w-1/4 bg-gray-100 p-6 shadow-lg'>
        <h3 className='text-xl font-bold mb-4'>Blog Titles</h3>
        <ul>
            {ut.length > 0 ? (
             ut.map(([title,id])=>(
                <li key={id} 
                onClick={() => handleC(id)}
                className='cursor-pointer hover:text-red-600 hover:underline'>{title}</li>
            ))  
            ):(
                <p>No Blog found</p>
            )}
